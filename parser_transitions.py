...
def minibatch_parse(sentences, model, batch_size):
    ...
    # list of partial parses, one for each sentence in sentences
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    # unfinished parses, a shallow copy of partial parses
    unfinished_parses = partial_parses[:]
    ...
    while(len(unfinished_parses)):
        ...
        if(len(unfinished_parses) >= batch_size):
            minibatch= [unfinished_parses[u] for u in range(batch_size)]
        else:
            minibatch= unfinished_parses
        # Use the model to predict the next transition for each partial parse in the minibatch
        transitions = model.predict(minibatch)
        # Perform a parse step on each partial parse in the minibatch with its predicted transition
        for idx,partial_parse in enumerate(minibatch):
        ...
        for pp in unfinished_parses:
            if ( not ( (len(pp.stack) == 1) &  (len(pp.buffer) == 0) ) ):
                unfinished.append(pp)
            unfinished_parses= unfinished
